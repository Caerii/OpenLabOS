/**
 * WebcamDeviceAdapter — minimal local camera adapter for laptops and tablets.
 *
 * Production deployments can swap the synthetic frame source for a WebRTC or
 * platform capture backend; the cadence contract stays the same.
 */
import type {
  Capability,
  DeviceAdapter,
  DeviceSession,
  DeviceSessionOptions,
  Frame,
  SensorSample,
} from "./adapter-types.js";

const MINIMAL_JPEG = new Uint8Array([
  ,
]);

export type FrameSource = () => Uint8Array;

export interface WebcamAdapterOptions {
  id?: string;
  /** Target preview cadence in frames per second. */
  fps?: number;
  /** Injectable frame source for tests and offline replay. */
  frameSource?: FrameSource;
  /** Injectable sleep for deterministic cadence tests. */
  sleep?: (ms: number) => Promise<void>;
}

export class WebcamDeviceAdapter implements DeviceAdapter {
  readonly id: string;
  private readonly fps: number;
  private readonly frameSource: FrameSource;
  private readonly sleep: (ms: number) => Promise<void>;
  private active = false;

  constructor(opts: WebcamAdapterOptions = {}) {
    this.id = opts.id ?? "webcam@local";
    this.fps = Math.max(1, Math.min(60, opts.fps ?? 6));
    this.frameSource = opts.frameSource ?? (() => MINIMAL_JPEG);
    this.sleep = opts.sleep ?? ((ms) => new Promise((resolve) => setTimeout(resolve, ms)));
  }

  async capabilities(): Promise<Capability[]> {
    return ["camera", "preview"];
  }

  async health(): Promise<{ ok: boolean; detail?: string }> {
    return { ok: true, detail: this.active ? "streaming" : "idle" };
  }

  async open(opts: DeviceSessionOptions): Promise<DeviceSession> {
    const adapterId = this.id;
    const fps = this.fps;
    const frameSource = this.frameSource;
    const sleep = this.sleep;
    this.active = true;

    return {
      adapterId,
      sessionId: opts.sessionId,
      capabilities: opts.request,
      preview: () => cadenceLoop({ adapterId, fps, frameSource, sleep }),
      sensors: () => emptyAsyncIterable(),
      invoke: async (capability, op) => {
        if (capability === "camera" && op === "status") {
          return { streaming: true, fps };
        }
        throw new Error(`unsupported webcam invoke: ${capability}.${op}`);
      },
      close: async () => {
        this.active = false;
      },
    };
  }
}

async function* cadenceLoop(opts: {
  adapterId: string;
  fps: number;
  frameSource: FrameSource;
  sleep: (ms: number) => Promise<void>;
}): AsyncIterable<Frame> {
  const intervalMs = 1000 / opts.fps;
  let seq = 0;
  while (true) {
    yield {
      seq: seq++,
      at: new Date().toISOString(),
      bytes: opts.frameSource(),
      source: { adapterId: opts.adapterId, format: "jpeg" },
    };
    await opts.sleep(intervalMs);
  }
}

async function* emptyAsyncIterable(): AsyncIterable<never> {
  /* yields nothing */
}

export type {
  Capability,
  DeviceAdapter,
  DeviceSession,
  DeviceSessionOptions,
  Frame,
  SensorSample,
} from "./adapter-types.js";
