...
All app-level code is in the `openlabos_voice/` package. In general, simple agents can be constructed with a single `agent.py` module. Additional modules can be added, but you must retain `agent.py` as the entrypoint (see the associated Dockerfile for how this is deployed).
